report 50008 "Payslip"
{
    ...
    dataset
    {
        dataitem("Res. Ledger Entry"; "Res. Ledger Entry")
        {
            ...
            Column(Employee_Major_Location; Employee."Major Location") { }
            ...
            Column(DeductionCode31; DeductionCode[31]) { }
            Column(DeductionCode32; DeductionCode[32]) { }
            Column(DeductionCode33; DeductionCode[33]) { }
            Column(DeductionCode34; DeductionCode[34]) { }
            Column(DeductionCode35; DeductionCode[35]) { }
            Column(DeductionCode36; DeductionCode[36]) { }
            Column(DeductionCode37; DeductionCode[37]) { }
            Column(DeductionCode38; DeductionCode[38]) { }
            Column(DeductionCode39; DeductionCode[39]) { }
            Column(DeductionCode40; DeductionCode[40]) { }
            ...
            Column(DeductionAmount31; DeductionAmount[31]) { }
            Column(DeductionAmount32; DeductionAmount[32]) { }
            Column(DeductionAmount33; DeductionAmount[33]) { }
            Column(DeductionAmount34; DeductionAmount[34]) { }
            Column(DeductionAmount35; DeductionAmount[35]) { }
            Column(DeductionAmount36; DeductionAmount[36]) { }
            Column(DeductionAmount37; DeductionAmount[37]) { }
            Column(DeductionAmount38; DeductionAmount[38]) { }
            Column(DeductionAmount39; DeductionAmount[39]) { }
            Column(DeductionAmount40; DeductionAmount[40]) { }
            ...
            begin
                IF PayrollPeriod = 0D THEN
                    ERROR('You must specify a payroll period');
                ...
                CurrReport.CREATETOTALS(BasicSalary, OtherEarnings, GrossPay, TaxablePay, PAYE, RSSF, RSSFEmployer, OtherDeductions, TotalDeductions
                                        , NetPay);
                CurrReport.CREATETOTALS(LocalServiceTax);

                IF (MajorLocation <> '') THEN
                    "Res. Ledger Entry".SETRANGE("Major Location", MajorLocation);
                        ...
                        END ELSE
                            ...
                            END ELSE
                                IF ResLedgerEntry."Payroll Entry Type" = ResLedgerEntry."Payroll Entry Type"::Deduction THEN BEGIN
                                    ...
                                    OR (ResLedgerEntry."ED Code" = 'NSSF-DIST') OR (ResLedgerEntry."ED Code" = 'NSSF-FARM') THEN
        ...
        layout
        {
            area(Content)
            {
                group(GroupName)
                {
                    ...
                    field("Major Location"; MajorLocation)
                    {
                        ApplicationArea = All;
                        ...
                        var
                            ...
                            ConfidentialList: Page Confidential;
                        begin
                            CLEAR(ConfidentialList);
                            Confidential.RESET;
                            Confidential.SETFILTER(Code, '<>%1', '');
                            Confidential.SETRANGE(Type, Confidential.Type::"Employee Location");
                            ConfidentialList.SETTABLEVIEW(Confidential);
                            ConfidentialList.LOOKUPMODE := TRUE;
                            IF ConfidentialList.RUNMODAL = ACTION::LookupOK THEN BEGIN
                                ConfidentialList.GETRECORD(Confidential);
                                MajorLocation := Confidential.Code;
                            END;
                        end;
                    }
                }
    ...
    begin
        ResNo2 := "Res. Ledger Entry"."Resource No.";
    end;
