report 50020 "Posted Bank Acc Recon Summary"
{
    Caption = 'Bank Account Reconciliation Summary';
    DefaultLayout = RDLC;
    RDLCLayout = './PostedBankAccReconciliationSummary.rdlc';
        ...
        dataitem("Bank Acc. Reconciliation"; "Bank Acc. Reconciliation")
        {
            ...
            dataitem("Bank Acc. Reconciliation Line"; "Bank Acc. Reconciliation Line")
            {
                ...
                begin
                    ...
                    //"BankNo." := "Bank Acc. Reconciliation"."Bank Account No.";
                    BankLedgerEntry.RESET;
                    BankLedgerEntry.SETRANGE(BankLedgerEntry."Bal. Account No.", "Bank Acc. Reconciliation Line"."Bank Account No.");
                    BankLedgerEntry.SETRANGE(BankLedgerEntry."Statement No.", "Bank Acc. Reconciliation Line"."Statement No.");
                    BankLedgerEntry.SETRANGE(BankLedgerEntry."Statement Line No.", "Bank Acc. Reconciliation Line"."Statement Line No.");
                    IF BankLedgerEntry.FINDSET THEN
                        REPEAT
                            ExternalDocNo := BankLedgerEntry."External Document No.";
                        UNTIL BankLedgerEntry.NEXT = 0;
                end;
            }

            //OutstandingCheckReal => Unpresented Checks
            ...
            dataitem(DifferenceAmount; "Bank Acc. Reconciliation Line")
            {
                DataItemLinkReference = "Bank Acc. Reconciliation";
                DataItemLink = "Bank Account No." = FIELD("Bank Account No."), "Statement No." = FIELD("Statement No.");

                Column(BankAccountNo_DifferenceAmount; DifferenceAmount."Bank Account No.") { }
                Column(StatementNo_DifferenceAmount; DifferenceAmount."Statement No.") { }
                Column(StatementLineNo_DifferenceAmount; DifferenceAmount."Statement Line No.") { }
                Column(DocumentNo_DifferenceAmount; DifferenceAmount."Document No.") { }
                Column(TransactionDate_DifferenceAmount; DifferenceAmount."Transaction Date") { }
                Column(Description_DifferenceAmount; DifferenceAmount.Description) { }
                Column(StatementAmount_DifferenceAmount; DifferenceAmount."Statement Amount") { }
                Column(Difference_DifferenceAmount; DifferenceAmount.Difference) { }
                Column(AppliedAmount_DifferenceAmount; DifferenceAmount."Applied Amount") { }
                column(Cleared_DifferenceAmount; DifferenceAmount.Cleared)
                {
                ...
                var
                    myInt: Integer;
                begin
                    DifferenceAmount.SetRange(DifferenceAmount.Type, DifferenceAmount.Type::Difference);
                end;
